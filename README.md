...
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, PieChart, Pie, LineChart, Line, Cell, Legend } from 'recharts';
...
const App = () => {
  ...
  const analysis = useMemo(() => {
    ...
    data.forEach(d => {
      const h = new Date(d.Order_Date).getHours() || 0;
    ...
    return { 
      ...
      hourly: Object.entries(hourly).map(([h, v]) => ({h: `${h}h`, v: v.toFixed(0)})),
